...
import { downloadFile, deleteFile } from '@/services/commom';
import { updataList, getFile } from '../service';
import Styles from '../index.less';

const FormItem = Form.Item;

const CreateModal = ({ dispatch, loadingFile }) => {
  const ref = useRef();
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const [fileList, setFile] = useState([]);
  const [fileNew, setFileList] = useState([]);
  const { res } = history.location.query;
  const resData = JSON.parse(res);

  const query = async () => {
    const response = await getFile({ id: resData.id });
  ...
  const deleteFilled = async (data) => {
    ...
    const response = await deleteFile('/file/downAndUpload/deleteFile', param);
