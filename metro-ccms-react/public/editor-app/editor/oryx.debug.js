...
var DataManager = {
  ...
  __persistShape: function (shape) {
    ...
    shapeData.each(function (data) {
      ...
      var value =
        data.type == 'resource' ? new ERDF.Resource(data.value) : new ERDF.Literal(data.value);

      // construct triple and add it to the DOM.
      DataManager.addTriple(
        new ERDF.Triple(shapeResource, { prefix: data.prefix, name: data.name }, value),
      );
    });
  },
  ...
  addTriple: function (triple) {
    ...
    if (!triple.subject.type == ERDF.LITERAL)
      throw (
        'Cannot add the triple ' + triple.toString() + ' because the subject is not a resource.'
      );
  ...
  __removeTriple: function (triple) {
    ...
    if (!triple.subject.type == ERDF.LITERAL)
      throw (
        'Cannot remove the triple ' + triple.toString() + ' because the subject is not a resource.'
      );
  ...
  query: function (subject, predicate, object) {
    ...
    return DataManager._triples.select(function (triple) {
      var select = subject
        ? triple.subject.type == subject.type && triple.subject.value == subject.value
        : true;
      if (predicate) {
      ...
      select =
        select &&
        (object ? triple.object.type == object.type && triple.object.value == object.value : true);
      return select;
    });
  },
};
      ...
      ResourceManager.__request(
        ...
        function () {
          ResourceManager.__resourceActionSucceeded(
            this,
            synchronize ? RESOURCE_SAVED | RESOURCE_SYNCHRONIZED : RESOURCE_SAVED,
            _id,
          );
        },
        function () {
          ResourceManager.__resourceActionFailed(
            this,
            synchronize ? RESOURCE_SAVED | RESOURCE_SYNCHRONIZED : RESOURCE.SAVED,
            _id,
          );
        },
        USE_ASYNCHRONOUS_REQUESTS,
...
ORYX.CONFIG.STENCILSET_HANDLER =
  ORYX.CONFIG.SERVER_HANDLER_ROOT + '/editor_stencilset?embedsvg=true&url=true&namespace=';
...
ORYX.CONFIG.CANVAS_MIN_HEIGHT = 200;
...
ORYX.Core.SVG.EditPathHandler = Clazz.extend({
  ...
  curvetoQuadraticAbs: function (x1, y1, x, y) {
    ...
    this.d = this.d.concat(
      ' Q' + pointsAbs[0] + ' ' + pointsAbs[1] + ' ' + pointsAbs[2] + ' ' + pointsAbs[3] + ' ',
    );
  },
  ...
  curvetoQuadraticRel: function (x1, y1, x, y) {
    ...
    this.d = this.d.concat(
      ' q' + pointsRel[0] + ' ' + pointsRel[1] + ' ' + pointsRel[2] + ' ' + pointsRel[3] + ' ',
    );
  },
  ...
  curvetoCubicSmoothAbs: function (x2, y2, x, y) {
    ...
    this.d = this.d.concat(
      ' S' + pointsAbs[0] + ' ' + pointsAbs[1] + ' ' + pointsAbs[2] + ' ' + pointsAbs[3] + ' ',
    );
  },
  ...
  curvetoCubicSmoothRel: function (x2, y2, x, y) {
    ...
    this.d = this.d.concat(
      ' s' + pointsRel[0] + ' ' + pointsRel[1] + ' ' + pointsRel[2] + ' ' + pointsRel[3] + ' ',
    );
  },
          ...
          if (this._rotate !== undefined) {
            if (this._rotationPoint)
              ...
              this.node.setAttributeNS(
                null,
                'transform',
                'rotate(' + this._rotate + ' ' + Math.floor(x) + ' ' + Math.floor(y) + ')',
              );
          }
        } else {
    ...
    try {
      ...
      if (refNode) {
        ...
        for (var j = 0; j < tspans.length; j++) {
          ...
          var refBoxLength =
            this._rotate != 0 && this._rotate % 180 != 0 && this._rotate % 90 == 0
              ? refbb.height
              : refbb.width;
...
ORYX.Core.Math.getPointOfIntersectionPointLine = function (lineP1, lineP2, point, onSegmentOnly) {
  ...
  var u =
    ((point.x - lineP1.x) * (lineP2.x - lineP1.x) + (point.y - lineP1.y) * (lineP2.y - lineP1.y)) /
    denominator;
...
ORYX.Core.Math.getDeterminant = function (m) {
  ...
  return (
    m.a * m.d * 1 + m.c * m.f * 0 + m.e * m.b * 0 - m.e * m.d * 0 - m.c * m.b * 1 - m.a * m.f * 0
  );
};
...
ORYX.Core.StencilSet.Property = Clazz.extend({
  ...
  construct: function (jsonProp, namespace, stencil) {
    ...
    jsonProp.id =
      jsonProp.id || ORYX.Log.error('ORYX.Core.StencilSet.Property(construct): Id is not defined.');
      ...
      if (jsonProp.complexItems && jsonProp.complexItems instanceof Array) {
        jsonProp.complexItems.each(
          function (jsonComplexItem) {
            ...
            ] = new ORYX.Core.StencilSet.ComplexPropertyItem(jsonComplexItem, namespace, this);
          }.bind(this),
        );
      }
    }
  ...
  addExtension: function (url) {
    new Ajax.Request(url, {
      ...
      onFailure: function (transport) {
        ORYX.Log.debug(
          'Loading stencil set extension file failed. The request returned an error.' + transport,
        );
      }.bind(this),
  ...
  /** @lends ORYX.Core.AbstractShape.prototype */
  {
    ...
    construct: function (options, stencil, facade) {
      ...
      if (this._stencil._jsonStencil.superId) {
        ...
        superStencilId =
          stencilId.substring(0, stencilId.indexOf('#') + 1) + stencil._jsonStencil.superId;
...
ORYX.Core.Canvas = ORYX.Core.AbstractShape.extend({
  ...
  focus: function () {
    try {
      // Get a href
      if (!this.focusEl) {
        this.focusEl = jQuery('body').append(
...
ORYX.Editor = {
  ...
  importJSON: function (jsonObject, noSelectionAfterImport) {
    ...
    if (
      ...
      jsonObject.stencilset.namespace !== this.getCanvas().getStencil().stencilSet().namespace()
    ) {
      alert(
        String.format(
    ...
    if (newShapeObject instanceof ORYX.Core.Edge && option.connectedShape) {
      ...
      if (option.connectedShape instanceof ORYX.Core.Node) {
        newShapeObject.dockers
          .first()
          .setReferencePoint(option.connectedShape.getDefaultMagnet().bounds.center());
  ...
  _handleMouseDown: function (event, uiObj) {
    ...
    var currentIsMovable =
      elementController !== null && elementController !== undefined && elementController.isMovable;
                      ...
                      switch (property.type()) {
                        case ORYX.CONFIG.TYPE_BOOLEAN:
                          ...
                          svgElem.setAttributeNS(
                            null,
                            'display',
                            !(prop === property.inverseBoolean()) ? 'inherit' : 'none',
                          );
                            ...
                            if (onclickAttr) {
                              if (prop && ('' + prop).length > 0) {
                                onclickAttr.textContent =
                                  "window.location = '../service/editor?id=" + prop + "_form'";
                              } else {
                                newFormFacade = this.facade;
                                onclickAttr.textContent =
                                  "displayNewFormDialog('" + this.resourceId + "');";
                              }
                            }
                        ...
                        case ORYX.CONFIG.TYPE_DIAGRAM_LINK:
                          ...
                          var hrefAttr = svgElem.getAttributeNodeNS(
                            'http://www.w3.org/1999/xlink',
                            'xlink:href',
                          );
                          if (hrefAttr) {
  ...
  getIntersectionPoint: function () {
    ...
    while (true) {
      ...
      var length = Math.sqrt(
        Math.pow(includePointX - excludePointX, 2) + Math.pow(includePointY - excludePointY, 2),
      );
...
ORYX.Core.Edge = {
  ...
  _update: function (force) {
    if (this._dockerUpdated || this.isChanged || force) {
      ...
      var oldHeight =
        this._oldBounds.height() === 0 ? this.bounds.height() : this._oldBounds.height();
        ...
        function (label) {
          if (label.getReferencePoint()) {
            ...
            if (!this.parent.initializingShapes) {
              ...
              var mx =
                isHor && ref.orientation && (ref.iorientation || ref.orientation).endsWith('r')
                  ? -label.getWidth()
                  : 0;
              var my =
                isVer && ref.orientation && (ref.iorientation || ref.orientation).startsWith('l')
                  ? -label.getHeight() + 2
                  : 0;
              label.setX(oiv.x + mx);
              label.setY(oiv.y + my);
  ...
  refreshAttachedNodes: function () {
    this.attachedNodePositionData.values().each(
      function (nodeData) {
        ...
        newNodePosition.x =
          startPoint.x + nodeData.relativDistanceFromDocker1 * (endPoint.x - startPoint.x);

        /* Calculate new y-coordinate */
        newNodePosition.y =
          startPoint.y + nodeData.relativDistanceFromDocker1 * (endPoint.y - startPoint.y);
      ...
      function (obj) {
        ...
        if (next) {
          if (next == targetShape) {
            ...
            this.dockers
              .last()
              .setReferencePoint({ x: next.bounds.width() / 2.0, y: next.bounds.height() / 2.0 });
  ...
  rollback: function () {
    this.shapesAsJson.each(
      function (shapeAsJson) {
        var shape = shapeAsJson.getShape();
        var parent =
          this.facade.getCanvas().getChildShapeByResourceId(shapeAsJson.parent.resourceId) ||
...
ORYX.Plugins.View = {
  ...
  zoom: function (factor) {
    ...
    var newScrollTop =
      ...
      Math.round((canvas.getHTMLContainer().parentNode.getHeight() - newHeight) / 2) +
      this.diff;
    var newScrollLeft =
      scrollNode.scrollLeft -
      Math.round((canvas.getHTMLContainer().parentNode.getWidth() - newWidth) / 2) +
      this.diff;
      ...
      if (!nearestLabel) {
        ...
        var diff = labels.collect(
          function (label) {
            ...
            var len = Math.sqrt(
              Math.pow(center.x - evtCoord.x, 2) + Math.pow(center.y - evtCoord.y, 2),
            );
    ...
    var commandClass = ORYX.Core.Command.extend({
      ...
      setBounds: function (shape, maxSize) {
        ...
        if (this.maxHeight > maxSize.height) {
          ...
          newBounds.b.y =
            shape.bounds.lowerRight().y + (maxSize.height - shape.bounds.height()) / 2;
        }
...
ORYX.Plugins.Save = Clazz.extend({
  ...
  updateTitle: function () {
    var value =
      window.document.title || document.getElementsByTagName('title')[0].childNodes[0].nodeValue;
    ...
    if (this.dragEnable) {
      ...
      if (!this.dragIntialized) {
        ...
        if (
          ...
          // Instanziate the dockCommand
          var commands = [
            new dockCommand(docker, position, this.containmentParentNode, this.facade),
          ];
            ...
            function (r) {
              return (
                (r.dockers.first().getDockedShape() == value ||
                  !this.moveShapes.include(r.dockers.first().getDockedShape())) &&
...
ORYX.Plugins.Overlay = Clazz.extend({
  ...
  setAttributes: function (node, attributes) {
    ...
    var attrMarkerValue = attributes.stroke
      ? attrValue.slice(0, attrValue.length - 1) + '; fill:' + attributes.stroke + ';}'
      : attrValue;
    var attrTextValue;
    if (attributes.fill) {
...
new (function () {
  ORYX.Plugins.BPMN2_0 = {
      ...
      dockers.each(function (docker) {
        ...
        if (!abs.isIncluded(docker.bounds.center())) {
          ...
          if (!bothAreIncluded) {
            var previousIsOver =
              index !== 0
                ? abs.isIncluded(docker.parent.dockers[index - 1].bounds.center())
                : false;
            var nextIsOver =
              index !== size - 1
                ? abs.isIncluded(docker.parent.dockers[index + 1].bounds.center())
    ...
    handleLayoutPool: function (event) {
      ...
      if (
        !(
          currentShape.getStencil().id().endsWith('Pool') ||
          currentShape.getStencil().id().endsWith('Lane')
        )
      ) {
        return;
      }
      ...
      if (lanes.length === 1 && this.getLanes(lanes.first()).length <= 0) {
        ...
        lanes
          .first()
          .setProperty('oryx-showcaption', lanes.first().properties['oryx-name'].trim().length > 0);
       ...
       */
        ...
        if (
          selection.length === 1 &&
          this.isResized(currentShape, this.hashedBounds[pool.id][currentShape.id])
        ) {
    ...
    adjustHeight: function (lanes, changedLane, propagateHeight) {
      ...
      while (++i < lanes.length) {
        if (lanes[i] === changedLane) {
          ...
          lanes[i].bounds.set(
            { x: 30, y: height },
            { x: lanes[i].bounds.width() + 30, y: lanes[i].bounds.height() + height },
          );
          ...
          while (++k < edges.length) {
            ...
            while (++l < edges[k].dockers.length) {
              ...
              var previousIsOverLane =
                l == 0 ? isOverLane : oldBounds.isIncluded(edges[k].dockers[l - 1].bounds.center());
              var nextIsOverLane =
                l == edges[k].dockers.length - 1
                  ? isOverLane
                  : oldBounds.isIncluded(edges[k].dockers[l + 1].bounds.center());
              var off = Object.clone(offset);
              ...
              if (
                isScaled &&
                isOverLane &&
                this.isResized(lanes[i], this.hashedBounds[pool.id][lanes[i].id])
              ) {
