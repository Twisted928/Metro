  ...
  function ($rootScope, $scope, $http, $modal, $timeout) {
    ...
    $scope.editorFactory.promise.then(function () {
      ...
      const getIcon = function (name) {
        if (name === '开始') {
          return 'iconpng/start.png';
        }
        if (name === '用户活动') {
          return 'iconpng/user.png';
        }
        if (name === '互斥网关') {
          return 'iconpng/hcwg.png';
        }
        if (name === '并行网关') {
          return 'iconpng/bxwg.png';
        }
        if (name === '结束任务') {
          return 'iconpng/end.png';
        }
        if (name === '池') {
          return 'iconpng/chi.png';
        }
        if (name === '泳道') {
          return 'iconpng/yd.png';
        }
        if (name === '文本注释') {
          return 'iconpng/text.png';
        }
        if (name === '顺序流') {
          return 'iconpng/jt.png';
        }
        if (name === '注释') {
          return 'iconpng/zs.png';
        }
      };
      /*
             StencilSet items
      ...
      $http({
        ...
        .success(function (data, status, headers, config) {
          ...
          const ignoreForPaletteDefinition = [
            'SequenceFlow',
            'MessageFlow',
            'Association',
            'DataAssociation',
          ...
          for (let stencilIndex = 0; stencilIndex < data.stencils.length; stencilIndex++) {
            ...
            let stencilItem = {
              ...
              icon: getIcon(data.stencils[stencilIndex].title),
      ...
      $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
        ...
        if (shapes && shapes.length > 0) {
          let selectedShape = shapes.first();
          let stencil = selectedShape.getStencil();
          ...
          if (
            ...
            $scope.selectedShape.getId() === selectedShape.getId()
          ) {
          ...
          for (let i = 0; i < properties.length; i++) {
            ...
            if (propertyConfig === undefined || propertyConfig === null) {
              console.log(
                `WARNING: no property configuration defined for ${key} of type ${property.type()}`,
              );
            } else {
      ...
      $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
        ...
        if (shapes && shapes.length == 1) {
          ...
          let a = $scope.editor.getCanvas().node.getScreenCTM();
          ...
          let shapeXY = bounds.upperLeft();

          let stencilItem = $scope.getStencilItemById(selectedShape.getStencil().idWithoutNs());
          let morphShapes = [];
          ...
          if (morphShapes && morphShapes.length > 0) {
            ...
            let morphButton = document.getElementById('morph-button');
          ...
          function (event) {
            if (event.property && event.property.key) {
              ...
              event.property.noValue =
                event.property.value === undefined ||
      ...
      $scope.morphShape = function () {
        $scope.safeApply(function () {
          let shapes = $rootScope.editor.getSelection();
          if (shapes && shapes.length == 1) {
            $rootScope.currentSelectedShape = shapes.first();
            let stencilItem = $scope.getStencilItemById(
              $rootScope.currentSelectedShape.getStencil().idWithoutNs(),
            );
            let morphShapes = [];
            for (let i = 0; i < $scope.morphRoles.length; i++) {
              if ($scope.morphRoles[i].role === stencilItem.morphRole) {
            ...
            let showSelectShapeDialog = function () {
      ...
      $scope.quickAddItem = function (newItemId) {
        $scope.safeApply(function () {
          ...
          if (shapes && shapes.length == 1) {
            ...
            let targetStencil = $scope.editor.getRules().connectMorph(args);
            if (!targetStencil) {
              return;
    ...
    $scope.updatePropertyInModel = function (property, shapeId) {
      ...
      let oldValue = shape.properties[key];
    ...
    $scope.getStencilItemById = function (stencilItemId) {
      for (let i = 0; i < $scope.stencilItemGroups.length; i++) {
        let element = $scope.stencilItemGroups[i];

        // Real group
        if (element.items !== null && element.items !== undefined) {
          let item = $scope.findStencilItemInGroup(stencilItemId, element);
          if (item) {
            return item;
          }
        } else {
    ...
    $scope.dropCallback = function (event, ui) {
      ...
      if ($scope.dragCanContain) {
        ...
        let screenCTM = $scope.editor.getCanvas().node.getScreenCTM();
        ...
        let parentAbs = $scope.dragCurrentParent.absoluteXY();
        pos.x -= parentAbs.x;
        pos.y -= parentAbs.y;
        ...
        if ($scope.quickMenu) {
          let shapes = $scope.editor.getSelection();
          if (shapes && shapes.length == 1) {
    ...
    $scope.dragCallback = function (event, ui) {
      if ($scope.dragModeOver != false) {
        let coord = $scope.editor.eventCoordinatesXY(event.pageX, event.pageY);
        ...
        let aShapes = $scope.editor.getCanvas().getAbstractShapesAtPosition(coord);
        ...
        if (aShapes.length == 1 && aShapes[0] instanceof ORYX.Core.Canvas) {
          ...
          let item = $scope.getStencilItemById(event.target.id);

          var parentCandidate = aShapes.reverse().find(function (candidate) {
            return (
              candidate instanceof ORYX.Core.Canvas ||
          ...
          if (item.type === 'node') {
            ...
            let parentItem = $scope.getStencilItemById(parentCandidate.getStencil().idWithoutNs());
            ...
            if (_canContain) {
              ...
              for (let i = 0; i < $scope.containmentRules.length; i++) {
                let rule = $scope.containmentRules[i];
            ...
            let canvasCandidate = $scope.editor.getCanvas();
            let canConnect = false;

            const targetStencil = $scope.getStencilItemById(
              parentCandidate.getStencil().idWithoutNs(),
            );
            if (targetStencil) {
              ...
              if (
                ...
                stencil.idWithoutNs() === 'DataAssociation' &&
    ...
    $scope.dragCallbackQuickMenu = function (event, ui) {
      if ($scope.dragModeOver != false) {
        ...
        if (stencil.type() === 'node') {
          ...
          let canContain = $scope.editor
            .getRules()
            .canContain({ containingShape: candidate, containedStencil: stencil });
          ...
          if (shapes && shapes.length == 1) {
            ...
            let targetStencil = $scope.getStencilItemById(curCan.getStencil().idWithoutNs());
            if (targetStencil) {
              let associationConnect = false;
              if (
                stencil.idWithoutNs() === 'Association' &&
...
KISBPM.CreateCommand = ORYX.Core.Command.extend({
  ...
  execute: function () {
    ...
    if (this.currentReference && this.position) {
      ...
      let containedStencil = this.containedStencil;
      let connectedShape = this.connectedShape;
      let bc = connectedShape.bounds;
      let bs = this.shape.bounds;
