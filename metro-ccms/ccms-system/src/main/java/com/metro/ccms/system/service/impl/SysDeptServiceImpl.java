...
import com.metro.ccms.system.mapper.SysUserDeptMapper;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import com.metro.ccms.common.annotation.DataScope;
...
//    @DataScope(deptAlias = "d")
    public List<SysDept> selectDeptList(SysDept dept)
    {
        ...
        if (list!=null && list.size()>0){
            for (SysDept sysDept:list){
                sysDept.setDeptName(sysDept.getDeptName()+"("+sysDept.getDeptCode()+")");
            }
        }
        return list;
    }
    ...
    public int insertDept(SysDept dept)
    {
        ...
        if (dept.getDeptCode()!=null && !"".equals(dept.getDeptCode()) && deptMapper.countDeptByCode(dept.getDeptCode())!=0){
    ...
    public int updateDept(SysDept dept)
    {
        ...
        if (!oldDept.getDeptCode().equals(dept.getDeptCode())){
            if (StringUtils.isNotBlank(dept.getDeptCode()) && deptMapper.countDeptByCode(dept.getDeptCode())!=0){
                throw new CustomException("部门编码已存在!");
            }
            if (StringUtils.isNotBlank(dept.getDeptCode()) && (deptMapper.countMemDeptByCode(dept.getDeptCode())!=0 || deptMapper.countUserDeptByCode(dept.getDeptCode())!=0)){
                throw new CustomException("部门编码已使用,无法修改!");
            }
        }
    ...
    {
        String updateBy = dept.getUpdatedBy();
        dept = deptMapper.selectDeptById(dept.getDeptId());
        dept.setUpdatedBy(updateBy);
